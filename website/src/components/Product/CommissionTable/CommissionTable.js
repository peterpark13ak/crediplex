...
import styles from './CommissionTable.css.js'
...
class CommissionTable extends Component {
  ...
  render()   {    
    ...
    if (!product || !product.commissions || product.commissions.length === 0) {
      return (
        <React.Fragment>
      ...
      <React.Fragment>
        <Card style={styles.Product}>  
          ...
          <Table style={styles.table}>
            ...
            <TableBody>
              {product.commissions.map((row, index) => (
                <TableRow key={index}>
...
export default withStyles(styles)(CommissionTable);
